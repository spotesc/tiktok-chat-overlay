...
class FetchSignedWebSocketFromEulerRoute extends route_1.Route {
    async call({ roomId, uniqueId, preferredAgentIds, sessionId, ttTargetIdc }) {
        ...
        let response;
        try {
            response = await this.webClient.webSigner.webcast.fetchWebcastURL('ttlive-node', roomId, uniqueId, this.webClient.clientParams?.cursor ?? undefined, resolvedSessionId, config_1.default.DEFAULT_HTTP_CLIENT_HEADERS['User-Agent'], preferredAgentIdsParam, resolvedTtTargetIdc, 
